...
class TDR:
    ...
    def delete_files(self, file_ids: list[str],
                     dataset_id: str,
                     submit_all_jobs_and_check_status_after: bool = False) -> None:
    ...
    def _yield_existing_datasets(self, filter: Optional[str] = None,
        ...
        while True:
            ...
            options = f"offset={offset}&limit={batch_size}&sort=created_date&direction={direction}{filter_str}"
            uri = f"{self.TDR_LINK}/datasets?{options}"
            response = self.request_util.run_request(uri=uri, method=GET)
...
class ReformatMetricsForIngest:
    ...
    def _format_relative_tdr_path(self, cloud_path: str) -> str:
        ...
        if self.cloud_type == GCP:
            ...
            if file_path_storage_container != self.workspace_storage_container:
                raise ValueError(
                    f"{cloud_path} storage container {file_path_storage_container} does\
                    not match workspace storage container {self.workspace_storage_container}.\
    ...
    def _check_and_format_file_path(self, column_value: str) -> Any:
        ...
        if file_to_uuid_dict isprovided then it will add existing uuid.
...
class SetUpTDRTables:
    """dict of dicts containing table info list expected columns are
    table_name, primary_key, ingest metadata, table_unique_id
    ...
    def _check_type_consistency(key_value_type_mappings: dict) -> None:
        ...
        for header, values_for_header in key_value_type_mappings.items():
            ...
            all_values_matching = all(  # noqa: E721
                type(v) == type_to_match_against for v in values_for_header if v is not None) # noqa
    ...
    def run(self) -> None:
        ...
        if auth_domain_list:
            ...
            if self.added_to_auth_domain:
                ...
                logging.info(
                    "Please add TDR SA account to auth domain group to\
                    allow access to workspace and then rerun with added_to_auth_domain=True")
                sys.exit(0)
...
class FilterOutSampleIdsAlreadyInDataset:
    ...
    def run(self) -> list[dict]:
        ...
        logging.info(
            f"Getting all {self.filter_entity_id} that already exist in table\
            {self.target_table_name} in dataset {self.dataset_id}")
